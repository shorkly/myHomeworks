...
(function () {
  ...
  var VanillaToast = function () {
    ...
    VanillaToast.prototype.show = function (text, option, callback) {
      ...
      if (!option) option = {}; // if immediately show option is on, cancel all previous toasts.

      if (option.immediately) self.cancelAll();
      var cancellationToken = new CancellationToken(); // enqueue
      ...
      self.queue.enqueue(function (next) {
        ...
        var duration = option.duration || constants["default"].duration; // close button setting

        self.element.closeButton.style.display = option.closeButton ? 'inline' : 'none'; // set text

        self.element.text.innerHTML = text; // set visible

        var s = self.element.toastBox.style;
        s.opacity = 0;
        s.display = 'inline-block'; // set styles

        _setStyle(self, option); // timeoutId


        var timeoutId = null; // duration timeout callback.

        var timeoutCallback = function timeoutCallback() {
          timeoutId = null; // release click clickHandler
    ...
    for (var item in constants) {
      (function (preset) {
        VanillaToast.prototype[preset] = function (text, option, callback) {
          if (!option) option = {}; // copy preset options
    ...
    function _hide(self, option, cancellationToken, callback) {
      if (!option) option = {}; // time setting

      var fadeDuration = option.fadeDuration || constants["default"].fadeDuration;
      var fadeInterval = option.fadeInterval || constants["default"].fadeInterval;
      var fadeStep = Math.min(fadeInterval / fadeDuration, 1); // set visible

      var s = self.element.toastBox.style;
      s.opacity = 1; // start fade out and call callback function.
  ...
  var CancellationToken = function () {
    ...
    CancellationToken.prototype.cancel = function () {
      ...
      while (copiedCallbacks.length) {
        copiedCallbacks.shift()();
      }
    };
  ...
  var TaskQueue = function () {
    ...
    TaskQueue.prototype.enqueue = function (job) {
      // enqueue.
      this.queue.push(job); // call execute.
    ...
    function dequeueAndExecute(self) {
      if (self.isExecuting) return; // Dequeue Job.

      var job = self.queue.shift();
      if (!job) return; //Execute Job.

      self.isExecuting = true; // Pass next job execute callback.
